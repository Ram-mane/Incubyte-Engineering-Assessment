...
| 8 | A commit message claiming a misconfiguration "passed every test in the suite; only asserting `current_user` caught it" | three of six tests failed on that run, both refusal tests among them. The accurate claim is narrower: the suite *before* that test existed passed, and `current_user` shortened the diagnosis rather than doing the detection. The overstatement was then quoted back as the project's strongest guardrail evidence and nearly became talk material | corrected in `DECISIONS.md` D102 with the failing output quoted. Same species as 7, from a different session, which is the finding: one occurrence is a slip, two is a pattern that needs the same scrutiny applied to verification claims as to code |
